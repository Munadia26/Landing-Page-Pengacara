...
const Navbar = () => {
  ...
  return (
    <header className="fixed top-0 left-0 w-full z-50 shadow-md">
      ...
      <div className={`hidden md:block w-full bg-[#c9a84c] text-[#0f1c2e] transition-all duration-500 overflow-hidden ${isScrolled ? 'max-h-0 opacity-0' : 'max-h-[200px] opacity-100'
        }`}>
        <div className="container mx-auto px-4 md:px-12 py-2 flex flex-col lg:flex-row items-center justify-between gap-y-2 lg:gap-4 text-[11px] md:text-xs font-bold uppercase tracking-tight">
          ...
          <div className="flex flex-col sm:flex-row items-center gap-2 sm:gap-4 lg:gap-8 shrink-0 w-full lg:w-auto justify-center">
            ...
            <a href={`mailto:${profile.contact.email}`} className="flex items-center gap-1.5 text-[11px] hover:text-white transition-colors">
      ...
      {/* 2. MAIN NAV (Navy) */}
      <nav className={`w-full transition-all duration-500 ${isScrolled || isMenuOpen ? 'bg-[#0f1c2e] py-2' : 'bg-[#0f1c2e]/95 py-3'
        }`}>
        ...
        <div className={`absolute top-full left-0 w-full bg-[#0f1c2e] border-t border-white/5 transition-all duration-300 lg:hidden overflow-hidden ${isMenuOpen ? 'max-h-screen opacity-100 py-6' : 'max-h-0 opacity-0'
          }`}>
